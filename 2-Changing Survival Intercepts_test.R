...
output = function(C,I){
  ...
  for (i in seq_len(nrow(Age.sex_groups))){
   ...
   find_intercept_surv <- function(g0) {
     p_surv_formula <- g0 + Inputs$gamma.param$g_Diabetes.Duration*mean(sub_df$Diabetes.duration) + Inputs$gamma.param$g_U*mean(sub_df$U) + Inputs$gamma.param$g_Diab.durationU*mean(sub_df$Diabetes.duration)*mean(sub_df$U)
    
     p_surv_est_pop <- exp(p_surv_formula) / (1 + exp(p_surv_formula))
  ...
  find_intercept_dementia = function(b0) {
   p_dementia_formula = b0 + Inputs$beta.param$b_Diabetes*mean(df_new$Diabetes) + Inputs$beta.param$b_U*mean(df_new$U) + Inputs$beta.param$b_Age*mean(df_new$Age.2016)
  
   p_dementia_est_overall = exp(p_dementia_formula)/(1+exp(p_dementia_formula))
  ...
  b0 = uniroot(find_intercept_dementia, interval = c(-50,50))$root
  
  lin.pred = with(df_new,exp(b0 + Inputs$beta.param$b_Diabetes*Diabetes + Inputs$beta.param$b_U*U + Inputs$beta.param$b_Age*Age.2016)) 
  ...
  Model_pop = glm(Dementia ~ Diabetes + Age.2016, data = df_survived, family = binomial(link = "logit"))
  coef = coef(Model_pop)
  ci = confint.default(Model_pop)
  Model.sum_pop = data.frame((cbind(coef, ci)[2:3,]))
  colnames(Model.sum_pop) = c("Beta", "Lower_CI", "Upper_CI")
  
  # Estimated effect of diabetes duration on dementia in the sample
  Model_sample = glm(Dementia ~ Diabetes + Age.2016, data = df_sample, family = binomial(link = "logit"))
  coef = coef(Model_sample)
  ci = confint.default(Model_sample)
  Model.sum_sample = data.frame((cbind(coef, ci)[2:3,]))
...
multiple_output = function(N,C,I,seeds){
  ...
  saveRDS(results, file = sprintf("Results_Scene%s_%s_N%s.RDA", I, C, N))
  invisible(results)
  }


#### 6. Perform Analysis ####
setwd("/Users/kelvin/Library/CloudStorage/GoogleDrive-kelvinzh@umich.edu/My Drive/SEGA/Projects/Cross-national Simulation/Results/Results_20250711/2-Changing Survival Intercepts")
my_random_seeds = scan("/Users/kelvin/Library/CloudStorage/GoogleDrive-kelvinzh@umich.edu/My Drive/SEGA/Projects/Cross-national Simulation/Results/Code_20250711/seeds.txt", what = numeric(), sep = ",",quiet = TRUE)
...
Outputlist <- list(
  US_2      = readRDS("Results_Scene2_US_N50.RDA"),
  England_2 = readRDS("Results_Scene2_England_N50.RDA"),
  India_2   = readRDS("Results_Scene2_India_N50.RDA"),
  China_2   = readRDS("Results_Scene2_China_N50.RDA"),
  Mexico_2  = readRDS("Results_Scene2_Mexico_N50.RDA"),
  US_3      = readRDS("Results_Scene3_US_N50.RDA"),
  England_3 = readRDS("Results_Scene3_England_N50.RDA"),
  India_3   = readRDS("Results_Scene3_India_N50.RDA"),
  China_3   = readRDS("Results_Scene3_China_N50.RDA"),
  Mexico_3  = readRDS("Results_Scene3_Mexico_N50.RDA")
)
...
writexl::write_xlsx(summary_stats, "summary_stats_ChangingSurvivalIntercepts.xlsx")
