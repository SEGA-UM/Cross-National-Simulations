...
output = function(C,I){
  ...
  df_new$Survival = unlist(lapply(1:nrow(df_new),function(i){ifelse(runif(1) < df_new$prob_death_est[i],0,1)}))
  ...
  df_new$Dementia= unlist(lapply(1:nrow(df_new),function(i){ifelse(runif(1) < df_new$p_dementia[i],1,0)}))
...
multiple_output = function(N,C,I){
  results = mclapply(1:N, function(i){
    output(C,I)
    },
  mc.cores = 5)
  saveRDS(results, file = paste0('Results_Scene1','_', C ,'_N',N,'.RDA',sep = ''))
  }


set.seed(20250711)
#### 6. Basic case (No bias) ####
setwd("/Users/kelvin/Library/CloudStorage/GoogleDrive-kelvinzh@umich.edu/My Drive/SEGA/Projects/Cross-national Simulation/Results/Results_20250711/1-Base Case")
...
Outputlist <- list(
  US_1      = readRDS("Results_Scene1_US_N250.RDA"),
  England_1 = readRDS("Results_Scene1_England_N250.RDA"),
  India_1   = readRDS("Results_Scene1_India_N250.RDA"),
  China_1   = readRDS("Results_Scene1_China_N250.RDA"),
  Mexico_1  = readRDS("Results_Scene1_Mexico_N250.RDA")
)
...
writexl::write_xlsx(summary_stats, "summary_stats_Base Case.xlsx")